...
- `pnpm dev:db-seed` — bring Postgres up, seed it from the terminal, and stop it again. Runs the same `seed()` as the admin dashboard's "Seed your database" button, so it clears the seeded collections (articles, volumes, pages, media, topics, forms, seed users) before re-creating them. Drizzle push builds the schema on an empty database, so it works straight after `dev:db-nuke`. Stop `pnpm dev` first: this command stops the Postgres container it shares. If the header or footer still show old nav afterwards, delete `.next/dev/cache` (#971)
