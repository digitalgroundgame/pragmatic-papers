...
describe("ShareButtons", () => {
  ...
  describe("native share", () => {
    ...
    it("shows native share button when navigator.share is available", async () => {
      ...
      render(<ShareButtons url={TEST_URL} title={TEST_TITLE} />)
      fireEvent.click(screen.getByRole("button", { name: "Share" }))
      expect(screen.getByRole("button", { name: "Open share menu" })).toBeTruthy()
    })

    it("calls navigator.share with title and url when native share button is clicked", async () => {
      ...
      expect(mockShare).toHaveBeenCalledWith({ title: TEST_TITLE, url: TEST_URL })
    })

    it("does not show native share button when navigator.share is unavailable", () => {
      Object.assign(navigator, { share: undefined })
      render(<ShareButtons url={TEST_URL} title={TEST_TITLE} />)
      fireEvent.click(screen.getByRole("button", { name: "Share" }))
      expect(screen.queryByRole("button", { name: "Open share menu" })).toBeNull()
    })
  })
})
