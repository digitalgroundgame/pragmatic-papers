...
function articleExcerpt(article: Article, volumeNumber: Volume["volumeNumber"]): string {
  if (article.meta?.description) return article.meta.description
  return `A new piece from Volume ${volumeNumber ?? ""}.`
}
...
export function VolumeArticleEmail({
  ...
  const excerpt = articleExcerpt(article, volume.volumeNumber)
