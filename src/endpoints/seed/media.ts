...
/**
 * Reads a file from the local filesystem and returns a File object compatible with Payload
 */
export async function fetchFileFromFile(filePath: string): Promise<File> {
...
/**
 * Creates a media document in Payload from a local file.
 *
 * Retries up to `maxAttempts` times on Drizzle internal errors (e.g.
 * `Symbol(drizzle:Columns)` or `_uuid` undefined), which can occur when
 * Next.js hot-reloads the adapter mid-run or when the DB is in a partially
...
export async function createMediaFromFile(
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    try {
      const file = await fetchFileFromFile(filePath)
      return await payload.create({
        collection: "media",
        data: { alt, ...(useAdditionalData ? additionalData : {}) },
        file,
